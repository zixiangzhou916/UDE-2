    ...
    for tid, files in files_reorg.items():
        for file in tqdm(files[::step_size], desc="SMPLX-to-MESH: TID = {:s}".format(tid)):
            ...
            if len(poses) == 3:
                ...
                pred = {
                    "body": result["vertices"][0].data.cpu().numpy(), 
                    "left": lh_result["vertices"][0].data.cpu().numpy(), 
                    "right": rh_result["vertices"][0].data.cpu().numpy()
                }
                faces = {
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='logs/ude/eval/exp3/mesh/t2m', help='')
