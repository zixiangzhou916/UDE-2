...
class UDEModel(nn.Module):
    ...
    def generate_audio_to_motion(
        ...
        # # input_embeds = sos_embeds.clone()
        # for motion_index in range(max_num_tokens):
        #     audio_index_i = int(motion_index * 16000 / 30)
        #     audio_index_j = audio_index_i + audio_block_size
        #     inp_audio_segment = audio[:, audio_index_i:audio_index_j]
        #     # Make sure the length of condition music piece equals audio_block_size
        #     if inp_audio_segment.size(-1) < audio_block_size:
        #         inp_audio_segment = audio[:, -audio_block_size:]
        #     raw_pred_logit = self.generate_next_motion_token_from_audio(
        #         audio=inp_audio_segment, 
        #         input_embeds=input_embeds, 
        #         task=task, part=part)
    ...
    def generate_speech_to_motion(
        ...
        pred_token, input_embeds = self.generate_motion_tokens_from_speech(
            ...
            temperature=temperature)
        pred_tokens.append(pred_token)
        # for _ in range(motion_block_size//4-primitive_token_size):
        #     raw_pred_logit = self.generate_next_motion_token_from_speech(
        #         audio=audio[:, :audio_block_size], 
        #         speaker_id=speaker_id, emotion=emotion, 
        #         input_embeds=input_embeds, 
        #         task=task, part=part
        #     )
        #     # For musix-to-motion task, we ignore <EOS> and <PAD> during generation, so we minimize the probability
    ...
    def sample_one_token(self, raw_pred_logit, topk=1, sas=False, temperature=1.0, task="t2m", part="body"):
        ...
        if not sas:
            ...
            if idx_out > self.pad_id:
                ...
                pred_token, *_ = self.quantizers["{:s}_{:s}".format(task, part)].semantic_aware_sampling(
                    idx_out.squeeze(dim=-1)-3, logits=raw_pred_logit[..., 3:].clone(), 
                    K=topk, temperature=temperature)
                pred_token += 3
            else:
...
if __name__ == "__main__":
    ...
    texts = [
        "A person is running forward quickly.", 
        "A person is running backward quickly.", 
        "A person is running forward slowly.", 
        "A person is running backward slowly.", 
        "A person is running left quickly.", 
        "A person is running right quickly.", 
        "A person walks forward quickly.",
        "A person walks backward quickly.",
        "A person walks forward slowly.",
        "A person walks backward slowly.",
        "A person walks left quickly.",
        "A person walks right quickly.",
    ]
    
    audio_files = [
        "../dataset/AIST++/aligned/gWA_sFM_cAll_d25_mWA4_ch05.npy", 
        # "../dataset/AIST++/aligned/gWA_sFM_cAll_d25_mWA2_ch03.npy", 
        # "../dataset/AIST++/aligned/gLH_sBM_cAll_d18_mLH2_ch06.npy", 
        # "../dataset/AIST++/aligned/gKR_sFM_cAll_d29_mKR5_ch14.npy", 
        # "../dataset/AIST++/aligned/gBR_sBM_cAll_d05_mBR4_ch10.npy", 
        # "../dataset/AIST++/aligned/gPO_sFM_cAll_d11_mPO2_ch10.npy", 
        # "../dataset/AIST++/aligned/gLO_sFM_cAll_d14_mLO5_ch14.npy", 
        # "../dataset/AIST++/aligned/gJB_sBM_cAll_d08_mJB4_ch10.npy", 
        # "../dataset/AIST++/aligned/gWA_sBM_cAll_d27_mWA4_ch04.npy", 
    ]
    
    speech_files = [
        "../dataset/BEAT_v0.2.1/aligned_20230331/2_scott_0_8_8.npy"
    ]
    
    audios = []
    emotions = []
    speaker_ids = []
    seq_len = int(160*16000/30)
    for name in audio_files:
        audio = np.load(audio_files[0], allow_pickle=True).item()
        audio = audio["audio_sequence"]
        audio = torch.from_numpy(audio).float().to(device).unsqueeze(dim=0)
        total_seq_len = audio.size(1)
        for i in range(0, seq_len, int(20*16000/30)):
            inp_audio = audio[:, i:i+seq_len]
            if inp_audio.size(1) < seq_len: continue
            audios.append(inp_audio)
    
    # for name in speech_files:
    #     data = np.load(speech_files[0], allow_pickle=True).item()
    #     audio = data["audio"]
    #     emotion = torch.tensor(data["emotion"][0]).view(1).long().to(device)
    #     speaker_id = torch.tensor(11).view(1).long().to(device)
    #     audio = torch.from_numpy(audio).float().to(device).unsqueeze(dim=0)
    #     total_seq_len = audio.size(1)
    #     for i in range(0, seq_len, int(20*16000/30)):
    #         inp_audio = audio[:, i:i+seq_len]
    ...
    # ckpt_path = "logs/ude/exp4/pretrained-1106/checkpoints/ude_UDE_final.pth"   # w/o semantic_enhancement
    # ckpt_path = "logs/ude/exp4/23-11-06-15-49-59/checkpoints/ude_UDE_E0300.pth" # w/ semantic_enhancement
    ckpt_path = "logs/ude/exp5/pretrained-1109/checkpoints/ude_UDE_E0300.pth" # w/ semantic_enhancement
    # ckpt_path = "logs/ude/exp9/pretrained-1112/checkpoints/ude_UDE_E0700.pth"     # w/o semantic_enhancement
    # ckpt_path = "logs/ude/exp9/23-11-13-16-10-37/checkpoints/ude_UDE_E0100.pth"     # w/ semantic_enhancement
    ...
    _, clip_emb, _, glob_emb, _ = model.encode_text(text=texts)
    # clip_emb, _, glob_emb = model.encode_audio(audio=audios)
    # clip_emb, _, glob_emb = model.encode_speech(speech=audios, ids=speaker_ids, emotion=emotions)
    print(clip_emb.shape, glob_emb.shape)
    
    output = {
        "pretrained_emb": clip_emb.data.cpu().numpy(), 
        "tuned_emb": glob_emb.data.cpu().numpy()
    }
    np.save("embeds_text_w_enhancement.npy", output)
